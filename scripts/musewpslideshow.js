...
(function (d) {
  ...
  Muse.Plugins.SlideShowLabel = {
    ...
    _updateLabels: function (b) {
      ...
      b._$sslpLabels.each(function () {
        for (var a = c._findAllTextNodes(this), b = a.length, i = 0, l = function (a) {
  ...
  Muse.Plugins.Lightbox = {
    ...
    initialize: function (b, c) {
      var a = this;
      d.extend(c, d.extend({}, a.defaultOptions,
        c));
    ...
    _initializeMarkup: function (b) {
      ...
      c.isResponsive && (m = m.map(function () {
        var a = d(this).parent();
      ...
      if (0 == g.length) {
        ...
        m.each(function (f, i) {
          ...
          if (j.css("position") !== "fixed")
            if (c.isResponsive) j.css({
              top: 0
            });
            else {
              var k = b._sslbpShownInitially ? j.offset() : {
                  top: Muse.Utils.getCSSIntValue(j, "top"),
                  left: Muse.Utils.getCSSIntValue(j, "left")
                },
                l = {
                  top: k.top - h.top
                };
              a ? l.top += Muse.Utils.getCSSIntValue(g,
                "padding-top") : l.left = k.left - h.left;
              j.css(l)
            }
        }).addClass("popup_element");
        var n = d("<div/>").attr("id", g.attr("id") || "").css({
            left: 0,
            top: 0,
            width: "auto",
            height: "auto",
          ...
          p;
        a && (p = d("<div/>"), c.elastic === "fullScreen" ? p.addClass("fullscreen") : c.elastic === "fullWidth" && p.addClass("fullwidth"), p.css({
        ...
        }), p.append(Muse.Utils.includeMEditableTags(f)), p.append(Muse.Utils.includeMEditableTags(m)), n.css({
          border: "none"
        }));
        g.removeAttr("id");
        var o = d("<div/>").addClass("overlayWedge").insertBefore(Muse.Utils.includeMEditableTags(j)[0]);
        n.append(Muse.Utils.includeMEditableTags(g.children().not("."
          + c.slideClassName)));
        ...
        f.css({
          position: c.elastic === "fullScreen" ? "relative" : "absolute",
          padding: 0,
          left: c.elastic === "fullWidth" ? "" : 0,
          top: 0,
          borderWidth: 0,
          background: "none"
        });
        c.elastic !== "fullScreen" && f.css({
          width: c.isResponsive ? l / b.$element.width() * 100 + "%" : i + "px",
          height: k
        });
        c.transitionStyle === "fading" && j.css({
          position: "absolute",
          left: 0,
          top: 0
        });
        var q;
        if (b._fstpPositionSlides
          || b._csspResizeFullScreenImages) q = function (a, d) {
        ...
        f = d("<div/>").addClass("LightboxContent").css({
          position: "absolute"
        }).append(a ? p : f);
  ...
  Muse.Plugins.ContentSlideShow = {
    ...
    initialize: function (b, c) {
      ...
      g && (WebPro.Widget.ContentSlideShow.slideImageIncludePlugin.initialize(b, c), Muse.Plugins.SlideShowLabel.initialize(b,
        c), Muse.Plugins.SlideShowCaptions.initialize(b, c));
      ...
      if (c.contentLayout_runtime === "lightbox") {
        ...
        Muse.Plugins.Lightbox.initialize(b,
          c)
      }
      ...
      d("body").on("muse_bp_activate", function (d, g, i) {
    ...
    _syncTargetHeights: function (b) {
      var c = 0;
      b._findWidgetElements("." + b.options.viewClassName).children().css("height",
        "").each(function () {
    ...
    _transformMarkup: function (b) {
      var c = b.options,
        a = b._findWidgetElements("." + c.viewClassName);
      c.transitionStyle !== "fading" ? (c = this._updateClipElement(b), b.$clipElement = a.css({
        position: "relative",
        top: "0",
        left: "0",
        margin: "0",
        overflow: "hidden"
      }).wrap(c).parent()) : c.isResponsive || (b = a.css("position"), c.elastic !== "fullScreen" && b !== "fixed" && a.css({
        width: "0",
        height: "0"
      }))
    },
    ...
    _attachBehavior: function (b) {
      ...
      d(window).on("orientationchange resize",
        i, c._onResize);
      c._onResize({
        data: i
      });
      ...
      if (g) {
        var l = g.$element;
        a.event === "mouseover" && l.bind("mouseenter", function () {
          var a = d(this);
          a.data("enter", !0);
        ...
        }) : a.deactivationEvent === "mouseout_both" && (l.bind("mouseleave", function () {
          var a = d(this),
            c = l.index(a),
            f = j.eq(c);
          a.data("enter", !1);
          a.data("setTimeout")
            || (a.data("setTimeout", !0), setTimeout(function () {
      ...
      g && f && (a.hideAllContentsFirst || g.$element.each(function (a) {
        a
          == b.slides.activeIndex ? d(this).addClass(f) : d(this).removeClass(f)
      ...
      b._csspIsImageSlideShow || (b.slides.bind("wp-panel-hide", function (a, b) {
        ...
        this != b.slides.activeElement
          || a.hideAllContentsFirst ? Muse.Utils.detachIframesAndObjectsToPauseMedia(d(this)) : Muse.Utils.attachIframesAndObjectsToResumeMedia(d(this))
      }));
    ...
    _hitTest: function (b, c) {
      ...
      return b.pageX
        >= a.x && b.pageX <= a.x + a.width && b.pageY >= a.y && b.pageY <= a.y + a.height
    },
    ...
    _resizeFullScreenImages: function (b, c, a) {
      c.each(function () {
        d(this).find("img").each(function () {
          this.complete && !d(this).hasClass(b.options.imageIncludeClassName)
            && b._csspPositionImage(this, a, b.options.elastic)
        })
      })
    },
    _setupImagePositioning: function (b, c, a, g) {
      ...
      c.each(function () {
        d(this).find("img").each(function () {
          var b = this;
          b.complete ? j._positionImage(b, a, g) : d(b).load(function () {
    ...
    _positionImage: function (b, c, a, g, j) {
      ...
      $img = d(b);
      i = i ? window.innerWidth ? window.innerWidth
        : f.width() : k ? $img.data("width") : $img.parent().closest(":not(.bc_ch_wrapper)").width();
      ...
      if (i == g && f == b) h.marginTop = "0px", h.marginLeft = "0px";
        ...
        if (c == "fillFrameProportionally") {
          if (a !== "off"
            || g > i && b > f) c = g / i, a = b / f, c < a ? (j = b / c, l = i) : (j = f, l = g / a)
  ...
  d.extend(WebPro.Widget.ContentSlideShow.slideImageIncludePlugin.defaultOptions, {
  ...
  WebPro.Widget.ContentSlideShow.prototype._getAjaxSrcForImage = function (b) {
    for (var c = d(window).data("ResolutionManager").getDataSrcAttrName(), a = c.length, g, j = 0; j < a; j++)
      if ((g = b.data(c[j])) && g.length) return g;
    return b.data("src")
  }
});;
(function () {
    ...
    if (-1 != a) {
      ...
      for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
        ...
        if ("generator" == d.getAttribute("name")) {
          "2017.0.0.363" != d.getAttribute("content") && Muse.assets.outOfDate.push("musewpslideshow.js");
          break
        }
      }
    }
