...
(function (d) {
  ...
  if (!Array.indexOf) Array.prototype.indexOf = function (c) {
  ...
  Muse.Utils.firstDescendant = function (c, a, b) {
    for (var d = 0; d < c.childNodes.length; d++) {
      ...
      if (f.nodeType == 1) {
        ...
        if (!b || !b.matches(f))
          if (f = Muse.Utils.firstDescendant(f, a, b)) return f
      }
    }
    return null
  };
  ...
  Muse.Utils.getStyleSheetRulesById =
    function (c, a) {
      var b = "#" + a.toLowerCase();
  ...
  Muse.Utils.allStyleSheetRulesFromOneSheet = function (c, a) {
    ...
    if (!j) b = !0, j = c.rules;
    if (!j) return null;
    for (var h = [], i = d(window), l = function (a) {
        if (4 != a.type) return !1;
        if (1 != a.media.length) {
          for (var c =
              0, b = 0; b < a.media.length; b++) {
            var d = null,
        ...
        c = i.data("muse-mq");
        if (!c) return !1;
        for (b = 0; b < a.media.length; b++)
          if (a.media[b] == c) return !0;
        if (a.media.mediaText && 0 <= a.media.mediaText.indexOf(c.replace(/\s/g, ""))) return !0;
        if (a.media.mediaText && (a = a.media.mediaText.replace(/\sand\s/g, "__and__").replace(/\s/g, "").split("__and__"), b = c.replace(/\sand\s/g, "__and__").replace(/\s/g, "").split("__and__"), a && a.sort && b && b.sort)) {
          "all"
          == a[0] && a.splice(0, 1);
          a.sort();
          b.sort();
      ...
      var m = j[k];
      if (l(m)) {
        if (m = Muse.Utils.allStyleSheetRulesFromOneSheet(m, a), null != m)
          for (var n = 0; n < m.length; n++) h.push(m[n])
      } else if (Muse.Utils.isDefined(m.selectorText))
        if (b) a(m.selectorText) && h.push(m);
        else
          for (var n = m.selectorText.split(/\s*,\s*/), p = 0; p < n.length; p++) a(n[p]) && h.push(m)
    }
  ...
  Muse.Utils.getStyleValue = function (c, a) {
    ...
    b || (document.defaultView ? b = document.defaultView.getComputedStyle(c,
      "").getPropertyValue(a) : c.currentStyle && (b = c.currentStyle[Muse.Utils.toCamelCase(a)]));
    ...
    if (!j || j == "_self") {
      ...
      if (h && j > f) {
        var b = g.data("scrollWrapper"),
          ...
          k = Math.min(h, j.top + (b
            && !b.isStandard() ? b.scrollTop() : 0)),
    ...
    var a = function () {
      ...
      if (a.length !== 0) {
        var j = a.offset(),
          ...
          l = Math.min(l, j.top + (h && !h.isStandard() ? h.scrollTop()
            : 0)),
          j = Math.min(f, j.left + (h && !h.isStandard() ? h.scrollLeft() : 0));
        a.scrollTo(j, l)
      }
    };
    if (d("body").hasClass("awaiting_bp_activate_scroll")) $window.one("scroll", function () {
      a()
    });
    else a()
  };
  ...
  Muse.Utils.redirectHyperlinkInNewTab =
    function (c, a) {
      if (Muse.Utils.redirectCancelled) setTimeout(function () {
        Muse.Utils.redirectCancelled = !1
      }, 0);
  ...
  Muse.Utils.addHyperlinkAnchor = function (c) {
    ...
    Muse.Utils.isIBE() || c.bind("click", function () {
      return !1
    })
  };
  Muse.Utils.addHyperlinkBlock = function (c) {
    ...
    a.bind("mouseup keyup", function (a) {
      Muse.Utils.isMouseLeftClick(a) && b.indexOf(this) != -1 ? a.ctrlKey || a.metaKey ? Muse.Utils.redirectHyperlinkInNewTab(c.href, a.shiftKey) : Muse.Utils.redirectHyperlink(c) : Muse.Utils.isMouseMiddleClick(a) && b.indexOf(this) != -1 ? Muse.Utils.redirectHyperlinkInNewTab(c.href,
        a.shiftKey) : Muse.Utils.isRedirectLinkKeyboardAction(a) && Muse.Utils.redirectHyperlink(c);
      return !1
    });
  ...
  Muse.Utils.prepHyperlinks = function (b) {
    ...
    d("a.nonblock").each(function () {
      var a = d(this);
      a.data("registeredNonBlockLink") !== !0 && (a.parent('[class~="sbg"]').length > 0 ? Muse.Utils.addHyperlinkAnchor(this) : (a.attr("class").match(/anim_(\w+)/) || this.href.indexOf("devicelock=") != -1) && d(this).bind("click", function () {
  ...
  Muse.Utils.needPIE = function (b) {
    ...
    else if (Muse.Utils.pieFunctionQueue.push(b), !Muse.Utils.pieLoading) Muse.Utils.pieLoading = !0, b = "scripts/pie.js", b[0] == "/" && (b = location.pathname.indexOf(".html") != -1 ? location.pathname.substring(0, location.pathname.lastIndexOf("/")) + b : location.pathname + b, b = b.replace(/\/+/g, "/")), d.ajax({
      url: b,
      dataType: "script",
      complete: function () {
  ...
  Muse.Utils.transformMarkupToFixBrowserProblemsPreInit = function () {
    ...
    d.browser.msie ? (d("html").addClass("ie"), d.browser.version < 8 && Muse.Utils.changeLItoDIVs(), d.browser.version < 9 && (Muse.Utils.monitorCheckboxes(), Muse.Utils.addRoundedCorners())) : d.browser.SafariMobile && d("body").css("-webkit-text-size-adjust",
      "none")
  };
  Muse.Utils.monitorCheckboxes = function () {
    ...
    d(".fld-checkbox input[type=checkbox]").each(function () {
      b(d(this))
    }).click(function () {
      b(d(this))
    });
    d(".fld-radiobutton input[type=radio]").each(function () {
      b(d(this))
    }).click(function () {
  ...
  Muse.Utils.transformMarkupToFixBrowserProblems =
    function () {
      ...
      d.browser.msie && d.browser.version <= 9 && (d.browser.version <= 9 && (Muse.Utils.addGradientFill(), Muse.Utils.addShadows()), d.browser.version < 9 && (Muse.Utils.applyIEFilterToPNGImages(), Muse.Utils.addRGBA(), Muse.Utils.removeEdgeAnimationBorderForIE78()));
      ...
      typeof window.matchMedia === "undefined"
        && typeof window.msMatchMedia === "undefined" && d("html").addClass("nomediaqueries");
  ...
  Muse.Utils.fixSVGImages = function () {
    var b = document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#Image", "1.1"),
      a = d("html");
    b || (a.addClass("nosvg"), d("body img").each(function () {
      var a = d(this),
  ...
  Muse.Utils.addProtocolToVimeoYoutubeIfNeeded = function () {
    ...
    window && window.location && window.location.protocol && window.location.protocol.indexOf && 0 <= window.location.protocol.indexOf("file") && d("iframe").each(function () {
      var a = d(this),
        g = a.attr("src"),
        j = a.attr("data-src");
  ...
  Muse.Utils.applyIEFilterToPNGImages = function () {
    d.browser.msie && d.browser.version < 9 && d("body *").not(".museBgSizePolyfill img,.f3s_top,.f3s_mid,.f3s_bot").each(function () {
      var b =
        d(this);
      if (!b.data("mu-ie-matrix") && (b.css("background-image").match(/\b.png/i) || this.nodeName && this.nodeName.toLowerCase() == "img" && b.attr("src").match(/\b.png/i))) {
        var a = b.css("filter");
        b.css("filter", a ? a + " progid:DXImageTransform.Microsoft.gradient(startColorstr=#00FFFFFF,endColorstr=#00FFFFFF)" : "progid:DXImageTransform.Microsoft.gradient(startColorstr=#00FFFFFF,endColorstr=#00FFFFFF)")
      }
    })
  };
  ...
  Muse.Utils.fixImageFramesWithRoundedCorners = function () {
    Muse.Browser.Features.checkCSSFeature("border-radius") && Muse.Browser.Features.checkCSSFeature("-webkit-border-radius") && d(".rounded-corners").each(function () {
      if (d(this).hasClass("clip_frame")) {
        var b =
          Muse.Utils.firstDescendant(this, Muse.Utils.Match.byNodeName("img"));
      ...
      Muse.Utils.needPIE(function () {
        ...
        if (!a.data("mu-ie-matrix")) {
          ...
          if (!g || !(g.toLowerCase().indexOf("opacity") > 0 && g.indexOf("=100") < 0)) {
            if (b.childNodes.length && !Muse.Browser.Features.checkCSSFeature("border-radius") && (g = Muse.Utils.firstChild(b, Muse.Utils.Match.byNodeName("img")))
  ...
  Muse.Utils.resizeHeight = function (b) {
    var a = {},
      g = function (g) {
        ...
        a[f] || (a[f] = !0, g.watch("height", function () {
          var a = d(this);
          "fixed" != a.css("position") && a.closest(b).height(a.outerHeight())
        }))
      };
    if (0 < d(".breakpoint").length) d("body").on("muse_bp_activate", function (a, f, h) {
      d(b, h).each(function () {
        d(this).css("height",
          "auto");
        g(d(this))
  ...
  Muse.Utils.initializeAnimations = function (b) {
    ...
    d(".animationContainer").each(function () {
      var b =
        d(this);
      Muse.Utils.isIBE() || this.contentDocument && "complete" == this.contentDocument.readyState ? a(b) : b.load(function () {
        a(b)
      })
    })
  };
  Muse.Utils.fixTransformRotations = function () {
    Muse.Browser.Features.checkCSSFeature("transform") || d("*[data-mu-ie-matrix]").each(function () {
      var b = d(this),
        a = b.parent(),
      ...
      a.css({
        "margin-bottom": "-=" + (a.innerHeight() - f),
        "margin-right": "-=" + (a.innerWidth() - h)
      });
      b.hasClass("actAsDiv") ? (b.wrap('<span class="actAsDiv rotateWrapper"></span>'), b.parent().css("float", b.css("float"))) : b.hasClass("actAsInlineDiv") ? b.wrap('<span class="actAsInlineDiv rotateWrapper"></span>') : b.wrap('<div class="rotateWrapper"></div>');
      b.parent().css({
        top: j,
        left: g,
        position: "relative",
  ...
  Muse.Utils.firstDefined = function () {
    for (var b = 0; b < arguments.length; b++)
      if (Muse.Utils.isDefined(arguments[b])) return arguments[b]
  };
  ...
  Muse.Utils.initWidget = function (b, a, g) {
    if (0 == d(".breakpoint").length) d(b).each(function () {
      g(d(this))
    });
    else {
      for (var j = 0; j < a.length; j++) {
        ...
        Muse.Utils._initWidgetQueue[f].push({
          id: b,
          fn: g
        })
      }
        ...
        var h = function (a, b, c, g) {
          ...
          if (Muse.Utils._initWidgetQueue[a]) {
            for (; Muse.Utils._initWidgetQueue[a].length;)
              if (b = Muse.Utils._initWidgetQueue[a].shift(), b = b.fn(d(b.id, c))) b.$bp = c, b.breakpoint = g;
  ...
  Muse.Utils.detachIframesAndObjectsToPauseMedia = function (b) {
    ...
    d("iframe, object", b).each(function () {
      ...
      if (!b.is("object") || !(d.browser.msie && d.browser.version < 9)) {
        ...
        c.$next = b.next();
        c.$parent = b.parent();
        d.browser.msie ? (c.html = b.wrap('<div id="deleteMeWrapper"/>').parent().html(), b.remove(),
          c.$parent.children("div #deleteMeWrapper").remove()) : (c.$node = b.clone(), b.remove());
        a.push(c)
      }
    });
    ...
    d("video", b).each(function () {
      if (d.browser.msie && d.browser.version == 9 && this.pause && this.getAttribute("autoplay") && this.readyState != 4) d(this).one("play", function () {
        this.pause()
      });
  ...
  Muse.Utils.attachIframesAndObjectsToResumeMedia = function (b) {
    ...
    if (a = b.data("paused"))
      for (g = 0; g < a.length; g++) j = a[g], j.playFn(j.$node);
  ...
  Muse.Utils.VimeoVideoHelper =
    function (b) {
      ...
      window.addEventListener ? window.addEventListener("message",
        h, !1) : window.attachEvent("onmessage", h, !1);
      h = function () {};
  ...
  (function (b) {
    ...
    h.swapSources = function (a, b, c) {
      var d = a.attr("data-" + b);
      d && !("src" == b && a.hasClass("ImageInclude") && a.attr("src").indexOf("images/blank.gif") == a.attr("src").length - 16 && a.parents(".SlideShowWidget").length) && ("src" == c && !a.attr("data-" + c) && a.attr("data-" + c, a.attr("src")), a.attr("src", d))
    };
    h.isRetina = function () {
  ...
  Muse.Utils.readCookie = function (b) {
    ...
    for (var a = document.cookie.split(";"), d = 0; d < a.length; d++) {
      for (var j =
          a[d]; j.charAt(0) == " ";) j = j.substring(1, j.length);
  ...
  Muse.Browser.Features.Touch = function () {
    if (navigator.maxTouchPoints > 0 || window.matchMedia && window.matchMedia("(-moz-touch-enabled)").matches) return {
      ...
      for (var b = 0, a = Muse.Browser.domPrefixes.length; b < a; b++) {
        ...
        if (d + "MaxTouchPoints" in navigator && navigator[d + "MaxTouchPoints"]) return d = d.toUpperCase(), {
          Start: d + "PointerDown",
          End: d + "PointerUp",
          Move: d + "PointerMove",
          Listener: function (a) {
  ...
  Muse.Browser.Features.checkCSSFeature = function (b, a) {
    ...
    for (var d = d.charAt(0).toUpperCase() + d.substr(1), j = 0, f = Muse.Browser.domPrefixes.length; j < f; j++)
      if (Muse.Browser.domPrefixes[j] + d in a.style) return Muse.Browser.domPrefixes[j];
    return !1
  };
  ...
  Muse.Browser.Bugs.ClearNeedsOuterWidth = function () {
    ...
    b.id = "mbbcnow00";
    b.innerHTML = '<div>a</div><style type="text/css">#mbbcnow00{position:absolute;top:-9999px;left:-9999px;visibility:hidden;} #mbbcnow01{width:1px;margin-right:-9999px;float:left} #mbbcnow02{clear:left;}</style>';
  ...
  (function (b) {
    ...
    j.prototype.init = function (f) {
      ...
      if (0 != this.$verticalSpacer.length) {
        ...
        this.pageMinHeight = Muse.Utils.getCSSIntValue(f, "padding-top") + Muse.Utils.getCSSIntValue(f, "min-height") + Muse.Utils.getCSSIntValue(f,
          "padding-bottom");
        ...
        a.resize(function () {
          if (i) {
            var b = a.width();
            l ? (j.splice(0, j.length), j.push(b), l = !1, setTimeout(function () {
              l = !0
            }, 200)) : j[j.length - 1] != b && (j.push(b), 3 < j.length && j[j.length - 3] == j[j.length - 1] && (d.addClass("always_vert_scroll"), i = !1))
          }
          n.doUpdate()
        });
        this.doUpdate()
    ...
    j.prototype.doUpdate = function () {
      if (this.enabled
        ...
        this.$verticalSpacer.css({
          height: "calc(" + (b.browser.SafariMobile ? a.height() + "px" : "100vh") + " - " + (this.contentAboveSpacer + this.contentBelowSpacer) + "px " + (0 < f ? " + " : " - ") + Math.abs(f) + "px)",
          "min-height": h ? f + this.minHeight + "px" : ""
        });
        f = (h ? f + this.minHeight : Muse.Utils.getCSSIntValue(this.$verticalSpacer, "min-height"))
    ...
    f.prototype.init = function (f) {
      ...
      this.originalOffsetTop = Muse.Utils.tryParse(this.$spacer.attr("data-offset-top"),
        parseInt, 0);
        ...
        if (this.enabled)
          if (this.initialized) {
            ...
            if (b != this.spacerHeight) {
              ...
              if (b < this.spacerHeight && c == this.$page.outerHeight(!0)) b = this.spacerHeight, this.updateScrollClass(b),
                this.$spacer.css("height", b);
              this.pageResizeWatchEnabled = !0
            }
  ...
  Muse.Utils.requestAnimationFrame = function () {
    return window.mozRequestAnimationFrame && window.mozRequestAnimationFrame.bind(window) || window.requestAnimationFrame && window.requestAnimationFrame.bind(window) || window.webkitRequestAnimationFrame && window.webkitRequestAnimationFrame.bind(window) || function (b) {
      window.setTimeout(b,
        20)
    }
  }();
  Muse.Utils.animationFrameFx = function (b) {
    var a = b.fx;
    b.extend(a, b.fx);
    var d, j = b(window).data("stickyFooter"),
      f = function () {
    ...
    if (-1 != a) {
      ...
      for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
        ...
        if ("generator" == d.getAttribute("name")) {
          "2017.0.0.363" != d.getAttribute("content") && Muse.assets.outOfDate.push("museutils.js");
          break
        }
      }
    }
